 ...
 */
    ...
    public function view_orders_table($org_id=false,$closed=false)
    {
        /*SELECT orders.id, orders.hash, orders.date_create, orgs.short_name, order_stages.stage_code, order_stages.date_end
FROM  `orders` 
JOIN orgs ON orgs.id = orders.org_id
JOIN order_stages ON orders.id = order_stages.order_id
WHERE org_id=*/
        $this->db->select('orders.id, orders.hash, orders.date_create, orgs.short_name, 
            order_stages.stage_code, order_stages.date_end,orders.org_id,orders.contacter')
                ->from('orders')
...
return $result;
    ...
    public function create_stage($order_id,$table,$stage_code,$date_start,$manager_id,$info,$cart_num=FALSE)
    {
        ...
        {
            echo 'EEEEEE kartridg uge vnesen ';sleep(1);echo 1; return false;
        }
        else
        {
    ...
    public function get_orders_stage($stage_code)
    {      
...
*/
        ...
        $this->db->select('orders.hash,orders.date_create,orders.contacter,order_stages.order_id, 
            ...
            order_stages.info,orders.paymethod')
                ->from('order_stages')->join('orders','orders.id=order_stages.order_id')
                ->join('orgs','orgs.id=orders.org_id')
...
// получение полного имени картриджа по его айди номеру из каталога    
    ...
    public function update_stage_orders($stage_code, $order_id, $action_flag, $date_start,$date_end,$info=false)
    {
        ...
        $data['manager_id']=$this->ion_auth->user()->row()->id;
    ...
    {
        $this->db->select('date_start,date_end,info,sort,action_flag')
                ->from('order_stages')
                ->where('order_id',$order_id)
                ->where('stage_code',$stage_code);
    ...
    {      
        ...
        $this->db->select('orders.id, orders.date_create, orders.manager_id, orders.contacter, 
            orders.phone, orders.phonemob, orders.email as email2, orders.adres, orgs.short_name, orders.other_info,
            orders.org_id, orgs.tel,orders.paymethod,orders.discount')
                ->from('orders')
                ->join('orgs','orgs.id=orders.org_id');
